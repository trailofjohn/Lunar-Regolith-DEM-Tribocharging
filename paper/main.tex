...
The computational methodology integrates a DEM simulation framework to resolve the granular flow dynamics with a Python-based post-processing architecture to quantify triboelectric charge acquisition. As illustrated in Figure \ref{fig:paper_method_flowchart}, the computational workflow is structured in three sequential, interdependent phases:
\begin{enumerate}
...
\centering
    ...
    \node [draw, rectangle, fill=blue!10, text width=7.5cm, align=center, rounded corners, minimum height=1.6cm] (step1) {
        \textbf{Phase 1: Cohesion Calibration}\\
        Cylinder-Lift AoR Sweep ($N$=5k to 20k)\\
        Target $\theta = 38.3^\circ \pm 2^\circ \rightarrow$ Lock $k_c = 285\text{k J/m}^3$
    };
    \node [draw, rectangle, fill=green!10, text width=7.5cm, align=center, rounded corners, minimum height=1.6cm, below=0.6cm of step1] (step2) {
        \textbf{Phase 2: Charging Calibration}\\
        Vibratory Plate Run (Earth Baseline)\\
        Target $Q/M = 0.5\ \mu\text{C/g} \rightarrow$ Calibrate $k_d = 3.29 \times 10^{-5}\text{ C/J}$
    };
    \node [draw, rectangle, fill=red!10, text width=7.5cm, align=center, rounded corners, minimum height=1.6cm, below=0.6cm of step2] (step3) {
        \textbf{Phase 3: Parametric Production Sweeps}\\
        6 Cases (Earth vs Moon $\times$ 3 Amplitudes)\\
        Extract Coordination Number \& $Q/M$ Charging Matrix
    };
    \draw [->, line width=1.2pt] (step1) -- (step2);
    \draw [->, line width=1.2pt] (step2) -- (step3);
    \end{tikzpicture}
    \caption{Three-phase computational methodology flowchart detailing the SJKR cohesion calibration, collision-energy-based charging calibration, and parametric reduced-gravity production sweeps.}
    \label{fig:paper_method_flowchart}
\end{figure}
...
The transient bed fluidization state is diagnosed via the coordination number (CN) in LIGGGHTS, presented in Figure \ref{fig:composite_flow}a. Under Earth gravity, the lowest tested vibration amplitude of $0.5\text{ mm}$ is sufficient to fully fluidize the bed (CN $< 0.5$). On the Moon, however, the elevated $Bo_g$ causes the same $0.5\text{ mm}$ amplitude to fail, resulting in a jammed bed (CN $> 2.0$) with low mass flow. Restoring a fluidized flow requires doubling the vibration amplitude to $2.0\text{ mm}$ to supply enough mechanical energy to rupture SJKR cohesive contacts.
...
When operating at an equivalent amplitude of 1.0 mm, the lunar simulation produces a significantly lower final charge-to-mass ratio of 0.10 $\mu$C/g, representing a severe 80\% reduction compared to the terrestrial baseline (0.50 $\mu$C/g). The $Q/M$ scaling behavior across the full parametric sweep is summarized in Figure \ref{fig:composite_charging}c. Even when the vibration amplitude is doubled to 2.0 mm, the lunar feeder achieves a final $Q/M$ of only 0.19 $\mu$C/g. This remains well below the 0.50 $\mu$C/g target benchmark established by \cite{Trigwell2009} for effective dry beneficiation, demonstrating that the six-fold reduction in gravity attenuates the particle-particle and particle-wall impact energies to such an extent that standard vibratory transport geometries are highly inefficient for tribocharging under lunar conditions. This critical finding underscores the necessity for more aggressive active fluidization designs (e.g., vertical paddle mixers or auxiliary mechanical agitators) to satisfy the charging requirements of electrostatic separators on the Moon. 
...
Earth Base & 9.81 & 0.5 & 40 & $0.31 \pm 0.04$ & Fluidized \\
Earth Target & 9.81 & 1.0 & 40 & $\mathbf{0.50 \pm 0.06}$ & Fluidized \\
Earth High & 9.81 & 2.0 & 40 & $0.78 \pm 0.13$ & Fluidized \\ \midrule
Lunar Base & 1.62 & 0.5 & 40 & $0.08 \pm 0.05$ & Jammed \\
Lunar Target & 1.62 & 1.0 & 40 & $0.10 \pm 0.09$ & Jammed / Poor Flow \\
Lunar High & 1.62 & 2.0 & 40 & $\mathbf{0.19 \pm 0.23}$ & Fluidized \\ \bottomrule
\end{tabular}
}
\end{table}
...
The results have profound implications for the design of regolith beneficiation plants on the Moon. Under lunar gravity, a standard flat-plate vibratory feeder operating at a terrestrial 1.0 mm amplitude and 40 Hz is physically unviable. It leads to cohesion-dominated clumping and jamming (CN $> 2.0$), and causes an **80\% reduction in tribocharging** (from 0.50 $\mu$C/g on Earth to only 0.10 $\mu$C/g on the Moon). 
...
To overcome this, lunar vibratory feeders must operate at much higher mechanical intensities. An operating amplitude of **$\ge 2.0\text{ mm}$** is required just to rupture the SJKR cohesive contacts and achieve granular fluidization. However, even under fluidization at 2.0 mm amplitude, the final charge-to-mass ratio ($0.19\ \mu\text{C/g}$) remains well below the $0.5\ \mu\text{C/g}$ experimental target required for successful mineral separation in standard free-fall electrostatic separators. 
...
\begin{enumerate}
    ...
    \item Under lunar gravity, a standard terrestrial vibratory feeder operates in a highly cohesive regime ($Bo_g \approx 8.04$). This triggers cohesion-dominated clumping and jamming at standard amplitudes (CN $> 2.0$), requiring an increased amplitude of $\ge 2.0\text{ mm}$ to break cohesive contacts and fluidize the bed.
    \item Gravitational acceleration reduction causes severe impact velocity damping, shifting the collision energy PDF significantly leftward. This shifts the average collision energy down by orders of magnitude, causing a massive **80\% charging reduction** (to 0.10 $\mu$C/g) under lunar gravity at a standard 1.0 mm amplitude.
    \item Standard terrestrial vibratory feeders are insufficient to meet the charging requirements of electrostatic separators on the Moon. Next-generation systems must incorporate active mechanical mixing geometries or corrugated micro-channel feeder plates to achieve effective tribocharging in space applications.
\end{enumerate}
