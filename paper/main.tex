...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{Figures/fig_composite_calibration.png}
    \caption{\textbf{Cohesion calibration, scale convergence, and pile profile validation of LMS-1 lunar regolith simulant.} (a) Particle height distribution as a function of radial distance, comparing finite-size scaling effects between 5,000 and 20,000 particle systems against the $38.3^\circ$ bulk repose angle target. (b) Mathematical pile-profiling boundary detection and linear regression fit at the calibrated cohesion energy density ($k_c = 285,000\text{ J/m}^3$) for the 20,000 particle pile. (c) Three-dimensional settled conical heap at rest reconstructed from 5,000-particle LIGGGHTS coordinates. (d) Three-dimensional settled conical heap at rest reconstructed from converged 20,000-particle LIGGGHTS coordinates. All parameters enforce dynamic similarity via the Granular Bond number ($Bo_g$).}
    \label{fig:composite_calibration}
\end{figure}

The post-processed slope profiles from the mathematical regression algorithm and visual verification of the three-dimensional settled rest states are integrated in Figure \ref{fig:composite_calibration}b, \ref{fig:composite_calibration}c, and \ref{fig:composite_calibration}d. Once the calibration criterion was met, the $k_c$ value of $285,000\text{ J/m}^3$ was locked for all subsequent gravity-dependent feeder simulations, as cohesion is an invariant material property.
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Coupled triboelectric charging performance and mechanical collision mechanics.} (a) Transient evolution of the macroscopic charge accumulation, showing the average charge per particle (with standard deviation bounds in pico-Coulombs, pC) under Earth and Moon gravity fields. (b) Smooth Kernel Density Estimation (KDE) probability density functions of the particle-particle and particle-wall collision energy distributions, highlighting the severe energy attenuation triggered by the six-fold gravity reduction.}
    \label{fig:composite_charging}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.65\linewidth]{Figures/fig_parametric_qm.png}
    \caption{\textbf{Parametric charge-to-mass ratio ($Q/M$) sweep for terrestrial and lunar feeder designs.} The average final charge-to-mass ratio ($Q/M$) is plotted as a function of vibration amplitude under Earth ($9.81\text{ m/s}^2$) and Moon ($1.62\text{ m/s}^2$) gravity fields, evaluated against the experimental Trigwell baseline target range (shaded area). Error bars represent standard deviation across independent statistical seeds.}
